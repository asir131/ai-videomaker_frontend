...
const VoiceGenerator = () => {
  ...
  const [selectedVoiceId, setSelectedVoiceId] = useState(
    VOICES[VOICE_PROVIDERS.ELEVENLABS][0].id,
  );
  ...
  const handleClearScript = () => {
    if (
      window.confirm(
        "Are you sure you want to clear the script? You can paste a new one.",
      )
    ) {
      setScript("");
      setEditedScript("");
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            Choose a voice and generate audio
          </p>
        </div>
      </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-indigo-100 dark:border-indigo-800">
              <div className="mb-3">
                ...
                <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                  {
                    editedScript.split(/\s+/).filter((word) => word.length > 0)
                      .length
                  }{" "}
                  words
                </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="text-center">
                    <div
                      className={`font-bold mb-1 ${isSelected ? "text-green-700 dark:text-green-400" : "text-gray-900 dark:text-white"}`}
                    >
                      {voice.name}
                    </div>
                    <div className="flex items-center justify-center gap-1 text-xs">
                      <span
                        className={`px-2 py-0.5 rounded-full ${isSelected ? "bg-green-200 dark:bg-green-800 text-green-700 dark:text-green-300" : "bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400"}`}
                      >
                        {voice.gender}
                      </span>
                      <span
                        className={`px-2 py-0.5 rounded-full ${isSelected ? "bg-teal-200 dark:bg-teal-800 text-teal-700 dark:text-teal-300" : "bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400"}`}
                      >
        ...
        {!isGeneratingVoice ? (
          ...
          >
            ...
            <span>
              {script
                ? "Generate Voiceover"
                : "Paste a script to generate voiceover"}
            </span>
          </button>
        ) : (
          <button
