...
const ImageGenerator = () => {
  ...
  const handleAudioReplacement = (e) => {
    ...
    if (file) {
      ...
      setAudioDuration(0); // Reset so it can be re-loaded
  ...
  // Sync scenes when audio duration changes (e.g., after upload or generation)
  useEffect(() => {
    if (audioDuration > 0 && voiceoverSource) {
      console.log("🎵 Audio duration updated, syncing scenes:", audioDuration);
      updateScenes(script, sceneCount, audioDuration, true);
    }
  }, [audioDuration, voiceoverSource]);
              ...
              <div
                ...
                onDrop={(e) => {
                  ...
                  if (file && file.type.startsWith('audio/')) {
                    handleAudioReplacement({ target: { files: [file] } });
                  }
                }}
              >
                ...
                <div className="text-center">
                  <h4 className="font-bold text-gray-900 dark:text-white">Click or drag to upload audio file</h4>
                  <p className="text-sm text-gray-500 mt-1">MP3, WAV, M4A, OGG, WebM, MP4 (Max 500MB)</p>
                </div>
              </div>
            </div>
          )}
                ...
                <div className="bg-white dark:bg-gray-800 border-2 border-slate-100 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
                  ...
                  <div className="relative h-24 bg-blue-50 dark:bg-indigo-900/20 rounded-xl border border-blue-100 dark:border-indigo-900/50 overflow-hidden px-2">
                    <div className="absolute inset-0 flex items-center px-4">
                      <div className="w-full h-1 bg-white/30 dark:bg-slate-700/50 absolute top-1/2 -translate-y-1/2 left-1" />
                      {[...Array(8)].map((_, i) => (
                        <div
                          key={i}
    ...
    <>
      ...
      >
        ...
        <div className="space-y-4 mb-8">
          ...
          {/* Upload Card */}
          <div
            className={`cursor-pointer rounded-2xl border-2 transition-all overflow-hidden ${voiceoverSource === 'upload'
            ...
            onClick={() => setVoiceoverSource(voiceoverSource === 'upload' ? null : 'upload')}
          >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Upload an audio file to generate visuals from
                  </p>
                </div>
              </div>
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${voiceoverSource === 'upload'
                ? 'border-blue-500 bg-blue-500'
                : 'border-gray-300 dark:border-gray-600'
                }`}>
                {voiceoverSource === 'upload' && <Check size={14} className="text-white" />}
              </div>
            </div>
          </div>
          {voiceoverSource === 'upload' && settingsPanel}

          {/* Generated Audio Card */}
          <div
            className={`cursor-pointer rounded-2xl border-2 transition-all ${voiceoverSource === 'generated'
              ? 'border-purple-500 bg-purple-50/10 dark:bg-purple-900/10'
              : ''
              }`}
            onClick={() => setVoiceoverSource(voiceoverSource === 'generated' ? null : 'generated')}
          >
            <div className="p-6 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                    Use Your Generated Voiceover
                  </h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {!generatedAudioUrl
                      ? "Generate a voiceover first to use this option"
                      : "Use the voiceover generated in the previous step"}
                  </p>
                </div>
              </div>
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${voiceoverSource === 'generated'
                ? 'border-purple-500 bg-purple-500'
                : 'border-gray-300 dark:border-gray-600'
                }`}>
                {voiceoverSource === 'generated' && <Check size={14} className="text-white" />}
              </div>
            </div>
          </div>
          {voiceoverSource === 'generated' && settingsPanel}
        </div>
